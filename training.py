...
from source.util.training_utils import select_experiment_seed, get_class_weights, record_model, get_next_model_idx, get_network_architecture, initialise_network, get_criterion, get_optimiser_scheduler, set_activation_function
...
class _SubsetWithTransform(torch.utils.data.Dataset):
    """Apply a per-subset transform to an underlying dataset that uses a ``.transform`` attribute."""
    ...
    def __getitem__(self, idx):
        if hasattr(self.dataset, 'transform'):
            previous_transform = self.dataset.transform
            self.dataset.transform = self.transform
            sample = self.dataset[self.indices[idx]]
            self.dataset.transform = previous_transform
            return sample
...
dataset_selections = {}
train_val_test_split_criteria = {}

if load_dataset == 1: # Pre-built dataset on the config (no CSV splitting pipeline)
    if not hasattr(cf, 'train_ID'):
        raise AttributeError(
            f"Dataset '{dataset_name}' is configured with load_dataset=1 but "
            f"source/config/{dataset_name}.py does not define 'train_ID'."
        )

    classes_ID = cf.classes
    classes_OOD = []
    num_classes = len(classes_ID)
    multi_label = getattr(cf, 'multi_label', False)

    transform_train = cf.transform_train[args.setting]
    transform_test = cf.transform_test[args.setting]
    df_train_full = cf.train_ID

    train_idx, val_idx = train_test_split(
        list(range(len(df_train_full))),
        test_size=args.valSize,
        stratify=df_train_full.targets,
        random_state=int(dataset_seed),
    )

    df_train = _SubsetWithTransform(df_train_full, train_idx, transform_train)
    df_validation = _SubsetWithTransform(df_train_full, val_idx, transform_test)
    ...
    #print(class_selections, demographic_selections, dataset_selections, train_val_test_split_criteria)
    ...
    multi_label = dataset['MultiLabel']
...
criterion = get_criterion(criterion_name=cf.criterion,label_smoothing=args.label_smoothing,pos_weight=pos_weight)
...
model_list_path = 'outputs/saved_models/model_list.csv'
Model_details = [get_next_model_idx(model_list_path),file_name+'-'+str(seed),seed,dataset_name,args.setting[-1],args.net_type,args.depth,args.widen_factor,args.dropout,0,0,
...
record_model(model_list_path, Model_details)
