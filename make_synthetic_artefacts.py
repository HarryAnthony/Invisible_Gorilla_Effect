...
class RandomErasing_text(RandomErasing_square):
    ...
    def _get_bool_mask(self,img,h,w,i,j):
        ...
        while True:
            ...
            (text_width, text_height), (width_offset,height_offset) = font.font.getsize(self.text)
        ...
        # Draw the letter
        draw = PIL.ImageDraw.Draw(letter_canvas)
        draw.text(((letter_canvas.width - text_width) / 2 - width_offset,  (letter_canvas.height - text_height) / 2 - height_offset), self.text, font=font, fill='white')
        transform = T.Compose([T.ToTensor()])
        pos=(int(i),int(j))
        canvas.paste(letter_canvas,pos)
        canvas = canvas.resize((img.shape[-1], img.shape[-2]), PIL.Image.LANCZOS)
