...
def evaluate_mahalanobis_distance(net,idloader,oodloader,modules,modules_to_skip=[],use_cuda=True, verbose=True, 
             alpha=None,feature_combination=True,trainloader=None,OOD_dict={'name': ['Mahalanobis']},RMD=False,
             preprocess=False,magnitude=0,**kwargs):
    """
    Evaluate Mahalanobis distance score on the ID and OOD datasets.
    ...
    training_data_statistics = estimate_mean_precision(module_names=module_names,net=net,trainloader=trainloader,**kwargs)
    ...
    for OOD, loader in enumerate([idloader, oodloader]):
        ...
        for batch_idx, (inputs, target) in enumerate(loader):
            ...
            conf = calc_mahalanobis_score(inputs=inputs,net=net, modules=module_names,
                                                         training_data_statistics=training_data_statistics,
                                                         preprocess=preprocess,magnitude=magnitude,**kwargs)
    ...
    if feature_combination == True: #If combination of distances from different modules is used
        if alpha is None: #If alpha is not specified, use the training data to determine the standardisation values
            ...
            for batch_idx, (inputs, target) in enumerate(trainloader):
                print_progress(batch_idx + 1, l, prefix='Progress:', suffix='Complete', length=50,verbose=verbose)
                training_conf_batch = calc_mahalanobis_score(inputs=inputs,net=net, modules=module_names, 
        ...
        for module in module_names:
            prefix = "Relative Mahalanobis" if RMD else "Mahalanobis"
            magnitude_str = f", FGSM magnitude {magnitude}" if preprocess else ""
            OOD_dict['name'].append(f"{prefix} (module {module}{magnitude_str})")
...
mahalanobis_module_dict = {'ResNet18': {'MBM': {'Branch 1': {'modules': np.arange(6,17), 'modules_to_skip': []}, #{'modules': np.arange(4,19), 'modules_to_skip': []},
                                         'Branch 2': {'modules': np.arange(19,33), 'modules_to_skip': []}, #{'modules': np.arange(19,35), 'modules_to_skip': []},
                                         'Branch 3': {'modules': np.arange(35,49), 'modules_to_skip': []}, #{'modules': np.arange(35,51), 'modules_to_skip': []},
                                         'Branch 4': {'modules': np.arange(51,65), 'modules_to_skip': []}}, #{'modules': np.arange(51,69), 'modules_to_skip': []}},
                    ...
                    'vgg16': {'MBM': {'Branch 1': {'modules': np.arange(6,10), 'modules_to_skip': []},
                                            'Branch 2': {'modules': np.arange(11,17), 'modules_to_skip': []},
                                            'Branch 3': {'modules': np.arange(18,24), 'modules_to_skip': []},
                                            'Branch 4': {'modules': np.arange(25,31), 'modules_to_skip': []}},
                                'MBM_act_func_only': {'Branch 1': {'modules': np.arange(10,14), 'modules_to_skip': [11, 12]},
                                                    'Branch 2': {'modules': np.arange(17,24), 'modules_to_skip': [18,19,21,22]},
                                                    'Branch 3': {'modules': np.arange(27,34), 'modules_to_skip': [28,29,31,32]},
                                                    'Branch 4':{'modules': np.arange(37,44), 'modules_to_skip': [38,39,41,42]}}},
                    ...
                    'vit_b_32': {'MBM': {'Branch 1': {'modules': np.arange(4,26), 'modules_to_skip': []},
                                        'Branch 2': {'modules': np.arange(37,59), 'modules_to_skip': []},
                                        'Branch 3': {'modules': np.arange(70,92), 'modules_to_skip': []},
                                        'Branch 4': {'modules': np.arange(103,126), 'modules_to_skip': []}},
                                                            }}
