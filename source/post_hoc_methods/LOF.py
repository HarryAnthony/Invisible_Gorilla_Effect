...
def evaluate(net, idloader, oodloader, module, OOD_dict={'name': ['LOF']},**kwargs):
    """
    Evaluate LOF distance score on the ID and OOD datasets.
    ...
    module: int, list, tuple, numpy array, or str
        The index of the module after which to extract embeddings and apply LOF distance score. 
        If it is an array then LOF distance score is applied on several modules.
        If 'all' is passed, LOF distance score is applied to all modules of the network.
    OOD_dict: dict
        The dictionary containing details of OOD detection method. Is used to store tha name of the module(s) after which LOF distance score is applied.
    ...
    confidence = evaluate_LOF_distance(net, idloader, oodloader, modules=modules, OOD_dict=OOD_dict, **kwargs)

    return confidence


def evaluate_LOF_distance(net,idloader,oodloader,modules,modules_to_skip=[],num_classes=2,n_neighbours=20,LOF_mode='LOF_d',use_cuda=True, verbose=True, 
             alpha=None,feature_combination=True,trainloader=None,OOD_dict={'name': ['LOF']},**kwargs):
    """
    Evaluate LOF distance score on the ID and OOD datasets.
    ...
    modules: list, tuple, numpy array
        A list of modules after which to extract embeddings and apply LOF distance score.
    ...
    alpha: list
        A list of alpha values to use for combining the LOF distance scores from different modules. 
        If None, the LOF distance scores are standardised using the training data. Default: None.
    feature_combination: bool
        Whether to combine the LOF distance scores from the different modules into one score, or to
    ...
    LOF_models = {}  # Dictionary to store LOF models


    if LOF_mode == 'LOF_class':
        for module_count, module in enumerate(module_names):
            LOF_models[module] = {}  # Initialise a sub-dictionary for each module
            
            for class_idx in range(num_classes):
                all_features = latent_dim_data[module_count][class_idx].cpu().numpy()        
                lof = LocalOutlierFactor(n_neighbors=n_neighbours, novelty=True)
                lof.fit(all_features)  # Fit LOF model for the class
                
                # Store the LOF model in the dict under the module and class_idx
                LOF_models[str(module)][class_idx] = lof


    elif LOF_mode == 'LOF':
        for module_count, module in enumerate(module_names):
            all_features = np.concatenate([latent_dim_data[module_count][i].cpu().numpy() for i in range(num_classes)], axis=0)
            LOF_models[module_count] = LocalOutlierFactor(n_neighbors=n_neighbours, novelty=True)
            LOF_models[module_count].fit(all_features)

    else:
        raise ValueError('LOF_mode must be either "LOF" or "LOF_class"')
    

    with torch.no_grad():
        #Calculate the LOF metric for the ID and OOD data
        ...
        for OOD, loader in enumerate([idloader, oodloader]):
            ...
            for batch_idx, (inputs, target) in enumerate(loader):
                ...
                for module_count, module in enumerate(module_names):
                    ...
                    if LOF_mode == 'LOF_class':
                        LOF_class_score = [[] for _ in range(num_classes)]
                        for class_idx in range(num_classes):
                                LOF_class_score[class_idx] = LOF_models[str(module)][class_idx].decision_function(feature_means.cpu().numpy())
                        LOF_scores = np.min(LOF_class_score, axis=0)
                    elif LOF_mode == 'LOF':
                        LOF_scores = LOF_models[module_count].decision_function(feature_means.cpu().numpy())

                    conf_list[OOD][module_count].extend(LOF_scores.tolist())


    OOD_dict['name'] = []
    for module in module_names:
        OOD_dict['name'].append(str(LOF_mode)+' (n_neighbours '+str(n_neighbours)+ ', module '+str(module)+')')
...
def format_modules(modules):
    """
    Format the list of modules to apply LOF distance to a consise format.

    Parameters
    ----------
    modules : list
        List of modules to apply LOF distance to.
