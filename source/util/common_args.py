...
def create_parser():
    ...
    parser.add_argument('--setting', default='setting1', type=str,
                        help='Experiment setting (default: setting1). CheXpert: setting1–setting3; ISIC/MVTec: setting1–setting2.')
    ...
    parser.add_argument('--dataset', default='chexpert', type=str,
                        help='Dataset name (default: chexpert). Supported: chexpert, ISIC, MVTec.')
    ...
    parser.add_argument('--class_selections', '-c_sel', default="{'classes_ID': ['Cardiomegaly', 'No Finding'], 'classes_OOD': []}",
                        help='Class selections if args.setting is not in the dataset config (default matches CheXpert setting1).')
    parser.add_argument('--demographic_selections', '-d_sel', default="{}",
                        help='Demographic selections if args.setting is not in the dataset config.')
    parser.add_argument('--dataset_selections', '-dataset_s', default="{'no_support_device_selection': ['remove all images with support device'], 'seperate_patient_IDs': True}",
                        help='Dataset-specific selections if args.setting is not in the dataset config (default matches CheXpert setting1).')
    parser.add_argument('--train_val_test_split_criteria', '-split_sel', default="{'k_fold_split': True, 'k': 5, 'fold': 0}",
                        help='Train/val/test split criteria if args.setting is not in the dataset config (default matches CheXpert setting1).')
    ...
    parser.add_argument('--save_results_path', default='outputs/experiment_outputs',type=str,
                        help='Path to save OOD detection results.')
    ...
    parser.add_argument('--synth_scale', default=(0.01,0.01), type=tuple,
    ...
    parser.add_argument('--ood_dataset', default='ISIC', type=str,
                        help='Dataset for cross-dataset OOD evaluation (default: ISIC). Supported: chexpert, ISIC.')
    ...
    parser.add_argument('--ReAct_percentile', default=0.7, type=float,
                        help='Percentile of training activations at which to truncate (used for ReAct).')
    parser.add_argument('--DICE_sparsification_param', default=0.9, type=float,
    ...
    parser.add_argument('--ASH_variant', default='ASH-s', type=str,
                        help='Variant of ASH to use (ash-s, ash-b, ash-p).')
    parser.add_argument('--combine_metrics', default=False, type=bool,
                        help='Whether to combine metrics.')
    parser.add_argument('--first_edition', default=False, type=bool,
                        help='Whether to combine metrics.')
    parser.add_argument('--plot_metric', default=False, type=bool,
                        help='Whether to combine metrics.')
    #KNN
    parser.add_argument('--n_neighbours', default=20, type=int,
                        help='Number of neighbours for KNN or LoF')
    parser.add_argument('--KNN_mode', default='KNN',
                        help='KNN mode for OOD detection method [KNN,KNN class]')
    #kde
    parser.add_argument('--KDE_kernel', default='gaussian',
                        help='Kernel to use for KDE')
    parser.add_argument('--KDE_mode', default='KDE',
                        help='KDE mode for OOD detection method [KDE,KDE class]')
    #Residual
    parser.add_argument('--Feature_based_Dim',default=20,type=int,
                        help='Dimensionality to reduce feature-based methods to')
    #Nan
    parser.add_argument('--NaN_module',default='all',
                    help='Module to apply NaN')
    parser.add_argument('--mahalanobis_FGSM',default=0,type=float,
               help='FGSM magnitude for mahalanobis')
    #LOF
    parser.add_argument('--LOF_mode', default='LOF', 
                    help='LOF mode for OOD detection method [LOF,LOF class]')
    #Vim
    parser.add_argument('--ViM_alpha', default=0.25, type=float)
    #WeiPer
    parser.add_argument('--WeiPer_delta', default=1.0, type=float)
    parser.add_argument('--WeiPer_epsilon', default=0.01, type=float)
    parser.add_argument('--WeiPer_lambda1', default=2.5, type=float)
    parser.add_argument('--WeiPer_lambda2', default=0.1, type=float)
    parser.add_argument('--WeiPer_nbins', default=100, type=int)
    parser.add_argument('--WeiPer_scoring_method', default='KL_div', type=str)
    #ASH
    parser.add_argument('--ASH_percentile', default=0.6, type=float)
    #FB
    parser.add_argument('--Feature_based_modules', default=None, 
                        help='Module indices to use for feature based method')
    parser.add_argument('--Feature_based_alpha', default=None, 
                    help='Alpha to use for feature based method')
    #XOOD
    parser.add_argument('--XOOD_M_C', default=1.0, type=float)
    parser.add_argument('--XOOD_M_ensemble', default='False', type=str)
    #TAPUUD
    parser.add_argument('--TAPUUD_num_clusters_list', default=[3,5,7])
    #GradOrth
    parser.add_argument('--GradOrth_eps_threshold', default=0.9, type=float)
    #CORP
    parser.add_argument('--CORP_num_rff', default=100, type=int)
    parser.add_argument('--CORP_gamma', default=0.1, type=float)
    #NAC
    parser.add_argument('--NAC_sigmoid_alpha', default=1.0, type=float)
    parser.add_argument('--NAC_num_bins', default=100, type=int)
    parser.add_argument('--NAC_r', default=0.5, type=int)
    #GAIA
    parser.add_argument('--GAIA_mode', default='GAIA-Z', type=str)
    #NMD
    parser.add_argument('--NMD_per_layer', default=False)
    #MCDP
    parser.add_argument('--MCDP_dropout_rate', default = 0.3, type=float)
    parser.add_argument('--backbone_seed', default = 0, type=int)
    #FPI
    parser.add_argument('--FPI_threshold', default = 0.5, type=float)
    #DDPM
    parser.add_argument('--DDPM_metric', default = 'mse', type=str)
