...
class Dataset_selection_methods():
    ...
    def apply_selections(self,class_selections={'classes_ID':['All'],'classes_OOD':[]},demographic_selections={},dataset_selections={},train_val_test_split_criteria={}):
        ...
        """
        self.dataset['total_df'], self.dataset['classes_ID']  = self.select_classes(**class_selections)
    ...
    def select_classes(self, classes_ID, classes_OOD, atleast_one_positive_class=True,allow_multiple_positive_classes=False, replace_values_dict={np.nan: 0, -1: 0}, multi_label_bool=False):
        """
        ...
        multi_label_bool : bool, optional
            If True, the dataset is multi-label. The default is False.
        ...
        if multi_label_bool == False:
            ...
            self.reduce_class_labels(classes_ID,allow_multiple_positive_classes)
            self.reduce_classes()
            self.data = self.data.drop(columns=classes_ID)
        else:
            self.list_class_names = classes_ID

        return self.data, self.list_class_names
    ...
    def reduce_class_labels(self,classes_ID,allow_multiple_positive_classes):
        """
        Removes labels of classes which are not in the dataset and alters names to be compatible with multi-label data.
        ...
        for class_int, count in self.data.groupby('class').size().items():
            class_statement = []
            if allow_multiple_positive_classes == True:
                for i, col in enumerate(classes_ID):
                    positive = 'Pos' if class_int & (2 ** i) else 'Neg'
                    class_statement.append(f"{positive} {col}")
            else:
                if class_int == 0:
                    class_statement.append('Null class')
                else:
                    class_statement.append(classes_ID[int(np.log2(class_int))])

            self.list_class_names.append(' and '.join(class_statement))
    ...
    def split_train_test_val(self,k_fold_split=False,**kwargs):
        ...
        if 'is_split' in self.dataset and self.dataset['is_split'] == True:
            return None
        if 'class' in self.dataset['total_df'].columns: #Non multi-label data
            self.dataset['MultiLabel'] = False
            if k_fold_split == True:
                self.k_fold_cross_val(**kwargs)
            else:
                self.single_train_val_test_split(**kwargs)
        else: #Multi-label data
            self.dataset['MultiLabel'] = True
            if k_fold_split == True:
                self.k_fold_cross_val_multilabel(**kwargs)
            else:
                self.single_train_val_test_split_multilabel(**kwargs)   
    ...
    def k_fold_cross_val_multilabel(self, fold=0, k=5, dataset_seed=42):
        """
        A function for splitting the dataset into k folds for k-fold cross-validation, adapted for multi-label data.
        ...
        # Extracting the feature (X) and label (y) data from the dataset
        X = self.dataset['total_df'].index.values
        y = self.dataset['total_df'][self.list_class_names].values  # Assuming class columns are in 'list_class_names'
        #y = self.dataset['total_df'].drop(columns=['Path', 'patient_id']).values  # Assuming 'Path' and 'patient_id' columns exist
        
        # If there are fewer instances of any class than the number of folds, use non-stratified KFold
        if min(y.sum(axis=0)) < k:
            kf = KFold(n_splits=k, shuffle=True, random_state=int(dataset_seed))
            image_folds = list(kf.split(X))
        else:
            # Use MultilabelStratifiedKFold for stratification with multi-label data
            mskf = MultilabelStratifiedKFold(n_splits=k, shuffle=True, random_state=int(dataset_seed))
            image_folds = list(mskf.split(X, y))
        
        # Assign the train and validation splits based on the specified fold
        train_idx, val_idx = image_folds[fold]
        self.dataset['train_df'] = self.dataset['total_df'].iloc[train_idx]
        self.dataset['validation_df'] = self.dataset['total_df'].iloc[val_idx]

                
    def single_train_val_test_split_multilabel(self, valSize=0.1, testSize=0.1, balance_classes_in_val_test=True, dataset_seed=42):
        """
        A function for splitting the dataset into training, validation, and test sets, adapted for multi-label data.
        ...
        testSize: float
            The fraction of the dataset to use for the test set
        balance_classes_in_val_test: bool
            Whether to balance the classes in the validation and test sets
        dataset_seed: int
            The seed to use for splitting the dataset
        """
        if not (0 <= valSize < 1) or not (0 <= testSize < 1) or valSize + testSize >= 1:
            raise ValueError('valSize and testSize must be floats between 0 and 1, and valSize + testSize must be less than 1')

        # If the entire dataset is to be used for training, validation, or testing
        if valSize == 0 and testSize == 0:
            self.dataset['train_df'] = self.dataset['total_df']
            return
        elif valSize == 1 and testSize == 0:
        ...
        # Prepare data for multi-label stratification
        X = self.dataset['total_df'].index.values
        y = self.dataset['total_df'][self.list_class_names].values # Assuming class columns exclude 'Path' and 'patient_id'

        # Adjusted validation and test fractions based on the balanced dataset size
        test_val_combined_size = (valSize + testSize) #* frac_adjust
        
        # Split training and temp (combined val and test)
        msss = MultilabelStratifiedShuffleSplit(n_splits=1, test_size=test_val_combined_size, random_state=int(dataset_seed))
        train_index, temp_index = next(msss.split(X, y))

        self.dataset['train_df'] = self.dataset['total_df'].iloc[train_index]
        df_temp = self.dataset['total_df'].iloc[temp_index]
        


        if testSize == 0:
            self.dataset['validation_df'] = df_temp
        elif valSize == 0:
            self.dataset['test_df'] = df_temp
        else:
            # Separate temp set into validation and test using stratified shuffle split
            test_val_ratio = testSize / (valSize + testSize)
            msss_temp = MultilabelStratifiedShuffleSplit(n_splits=1, test_size=test_val_ratio, random_state=int(dataset_seed))
            test_index, val_index = next(msss_temp.split(df_temp.index.values, df_temp[self.list_class_names].values))
            
            self.dataset['validation_df'] = df_temp.iloc[val_index]
            self.dataset['test_df'] = df_temp.iloc[test_index]
