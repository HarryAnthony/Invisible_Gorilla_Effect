...
def calc_mahalanobis_score(inputs, net, modules, num_classes, training_data_statistics, magnitude=0,preprocess=False,RMD=False,use_cuda=True,**kwargs):
    """
    ...
    if requires_grad == False:
        with torch.no_grad():
            ...
            out_features = net.feature_extractor(inputs, module_names = modules)
    else:
        inputs = Variable(inputs,requires_grad=True)
        #out_features = feature_extractor(inputs)
        out_features = net.feature_extractor(inputs, module_names = modules)
    ...
    if preprocess:
        ...
        score_list = calc_mahalanobis_score(inputs_perturbed, net, modules, num_classes, training_data_statistics,preprocess=False,RMD=RMD,use_cuda=True,**kwargs)
...
def estimate_mean_precision(module_names, net, trainloader=None, num_classes=2,use_cuda=True,RMD=False,verbose=True,**kwargs):
    """
...
def estimate_mean_precision_layer(model, layer_extractor, num_classes, train_loader,use_cuda=True,RMD=False,verbose=False):
    """
    Compute sample mean and precision (inverse of covariance matrix) for each class in the training data.
    ...
    layer_extractor : torch.nn.Module
        The layer to be evaluated.
    num_classes : int
        Number of classes in the dataset.
    ...
    latent_dim_data = [0 for _ in range(num_classes)] #Stores the latent dimensions for each class

    print("Computing the sample mean and precisions for each class")
    l = len(train_loader)
    print_progress(0, l, prefix = 'Progress:', suffix = 'Complete', length = 50,verbose=verbose)

    for batch_idx, (data, target) in enumerate(train_loader): #Iterate through each image in training data
        print_progress(batch_idx + 1, l, prefix = 'Progress:', suffix = 'Complete', length = 50,verbose=verbose)
        ...
        with torch.no_grad():
            out_features = layer_extractor(data) #Get latent dimension data


        # get hidden features
        out_features = out_features.view(out_features.size(0), out_features.size(1), -1) #[batch_size, dim, image_combined]
        out_features = torch.mean(out_features.data, 2) #Mean for each dim for each image [batch_size,dim_mean]

        del data

        # construct the sample matrix
        for i in range(batch_size): #Go through each batch
            if num_sample_per_class[target[i]] == 0: #If there are no samples for the class in this layer
                latent_dim_data[target[i]] = out_features[i].view(1, -1) #Put in the form [1,dim_mean], save in list_features[layer][target]
            else:
                latent_dim_data[target[i]] \
                        = torch.cat((latent_dim_data[target[i]], out_features[i].view(1, -1)), 0)
            num_sample_per_class[target[i]] += 1
    ...
    if RMD:
        class_means, total_mean = get_class_means(latent_dim_data,RMD=True)
        class_precisions, total_precision = estimate_inv_covariance(latent_dim_data,class_means,RMD=True,total_mean=total_mean)
        total_mean_list = total_mean
        total_precision_list = total_precision
    else:
        class_means = get_class_means(latent_dim_data)
        class_precisions = estimate_inv_covariance(latent_dim_data,class_means)

    mean_list = class_means
    precision_list = class_precisions



    if RMD:
        return mean_list, total_mean_list, precision_list, total_precision_list
