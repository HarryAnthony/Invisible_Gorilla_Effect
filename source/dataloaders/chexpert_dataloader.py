...
def select_no_support_device_images(dataset,criteria=['remove all images with support device']):
    ...
    dataset: dict
        A dictionary containing the dataset information
    """
    nsd_list = np.loadtxt("data/CheXpert/annotations/no_support_device.txt", dtype=str)
...
class ChexpertSmall_heart(Dataset):
    def __init__(self, folder_dir, dataframe, transform, mask_dir):
        """
        CheXpert dataset with heart mask-based inversion.

        Parameters
        ----------
        folder_dir: str
            Root directory containing CheXpert images.
        dataframe: pandas.DataFrame
            DataFrame containing metadata, paths, and labels.
        transform: torchvision.transforms
            Transform to apply to the images.
        mask_dir: str
            Directory containing 224x224 heart mask PNGs.
        """
        self.image_paths = []
        self.targets = []
        self.patient_ids = []
        ...
        for row in dataframe.to_dict('records'):
            rel_path = row['Path']
            self.patient_ids.append(rel_path)
            self.image_paths.append(os.path.join(folder_dir, rel_path))
            self.targets.append(row['class'])

        assert len(self.targets) == len(self.image_paths), 'Mismatch in image and label counts.'
    ...
    def __getitem__(self, index):
        image_path = self.image_paths[index]
        image = Image.open(image_path).convert('L')  # Grayscale expected
        ...
        temp_path = image_path[len('.._.._data_CheXpert-v1.0-small_'):]
        mask_filename = temp_path.replace(".jpg", "_heart_mask.png").replace("/", "_")
        mask_path = os.path.join(self.mask_dir, mask_filename)


        if os.path.exists(mask_path):
            heart_mask = Image.open(mask_path).convert("L")
            heart_mask = heart_mask.resize(image.shape[1:], resample=Image.NEAREST)
            heart_mask = TF.to_tensor(heart_mask).squeeze(0)  # [H, W]
            heart_mask = (heart_mask > 0.5).float()


            # Apply Gaussian blur to soft the mask borders
            heart_mask_np = heart_mask.numpy()
            blurred_mask = gaussian_filter(heart_mask_np, sigma=0.75) #, truncate=5.0)  # Adjust sigma for softness
            blurred_mask = np.clip(blurred_mask, 0.0, 1.0)
            blurred_mask = torch.tensor(blurred_mask, dtype=torch.float32)
        else:
            blurred_mask = None


        # Invert only heart pixels
        if heart_mask is not None:
            image = image.clone()
            image = (image * (1 - blurred_mask)) + ((image) * blurred_mask  )/3  # Invert only where mask == 1


        transforms_image_2 = T.Compose([
            T.RandomRotation(degrees=15),
            T.RandomCrop(224, padding=20),
            T.Normalize(mean=[0.46362805],std=[0.28616473])])
        
        image = transforms_image_2(image)
